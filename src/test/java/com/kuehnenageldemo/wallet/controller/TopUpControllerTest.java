...
@SpringBootTest(classes = {TopUpController.class}, properties = {"spring.liquibase.enabled=false"})
...
public class TopUpControllerTest {
    ...
    @SneakyThrows
    public void okTest() {
        TopUpController.TopUpRequest topUpRequest = new TopUpController.TopUpRequest();
        topUpRequest.setWalletId(1L);
        topUpRequest.setAmount(new BigDecimal("123.45"));

        mockMvc.perform(
                MockMvcRequestBuilders.post("/api/wallet/top-up")
        ...
        Mockito.verify(
                ...
                Mockito.times(1)
        ).topUp(anyLong(), any(BigDecimal.class));
    }
